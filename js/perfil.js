...
var app = new Vue({
    ...
    methods: {
        ...
        getQtdPostUsuario: async function (apiPost) {
            apiPost += "?userId=" + this.userid // Albuns de um específico usuário
            await axios
                .get(apiPost)

                .then(response => {
                    if (response.data) {
                        this.qtdPosts = response.data.length
                    }
                })
                .catch(error => {
                    console.log("Houve um erro")
    ...
    async mounted() {
        ...
        await this.getQtdPostUsuario(urlPost)
    }
})
